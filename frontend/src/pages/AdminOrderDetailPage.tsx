...
const AdminOrderDetailPage: React.FC = () => {
  ...
  const handleStatusChange = async () => {
    ...
    if (selectedStatus === 'ENVIADO' && !shippingProof && !order.shippingProofUrl) {
    ...
    try {
        if (selectedStatus === 'ENVIADO' && shippingProof) {
            ...
            await customFetch.post(`/admin/orders/${order.id}/ship`, formData, {
                headers: {
            ...
            await customFetch.put(`/admin/orders/${order.id}`, { status: selectedStatus }, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
  ...
  const isButtonDisabled = () => {
    ...
    if (selectedStatus === 'ENVIADO' && !shippingProof && !order.shippingProofUrl) return true;
    return false;
  };
              ...
              {order.orderItems.map((item) => (
                <li key={item.id} className="flex justify-between items-center py-2 border-b">
                  <div className="flex items-center">
                    <img src={item.product.imageUrl} alt={item.product.name} className="h-16 w-16 rounded object-cover mr-4" />
                    <div>
                      <p className="font-semibold">{item.product.name}</p>
                      <p className="text-sm text-gray-600">Cantidad: {item.quantity}</p>
                    </div>
                  </div>
                  <p className="font-semibold">{formatPrice(item.price * item.quantity, 'Bs')}</p>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            {order.proofOfPaymentUrl && (
              <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                <h2 className="text-xl font-bold mb-4 text-gray-700">Comprobante de Pago</h2>
                <img 
            ...
            {order.shippingProofUrl ? (
              <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                <h2 className="text-xl font-bold mb-4 text-gray-700">Comprobante de Envío</h2>
                <img 
              ...
              selectedStatus === 'ENVIADO' && (
                <div className="bg-white p-6 rounded-lg shadow-md">
                  <label htmlFor="shippingProof" className="block text-lg font-medium text-gray-800 mb-2">
                  ...
                  {shippingProof && (
                    <div className="mt-4">
                        <h3 className="text-md font-medium text-gray-800 mb-2">Previsualización:</h3>
                        <img src={URL.createObjectURL(shippingProof)} alt="Shipping Proof Preview" className="w-full h-auto rounded-lg max-h-48 object-contain" />
                    </div>
                  )}
                </div>
              )
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {order.shippingAddress && (
                <div className="mt-4 pt-4 border-t">
                    <h3 className="text-lg font-bold mb-2">Información de Envío</h3>
                    <p><strong>Nombre Completo:</strong> {order.shippingAddress.firstName} {order.shippingAddress.lastName}</p>
            ...
            <div className="mt-6">
              <label htmlFor="status" className="block text-sm font-medium text-gray-700">Estado del Pedido</label>
              <select
                id="status"
